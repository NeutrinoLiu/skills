...
| [`bangya-slides`](bangya-slides/) | Builds a rich HTML slider deck: markdown Q&A rounds to settle content, a visual style negotiated in a parallel session against real mini-slides, wireframes, asset subagents, a click-through deck, headless render-checks, then `feedback.md` revision rounds. Copied verbatim from [Abaka-Skills/slides](https://github.com/Abaka-Skills/slides/tree/main/skills/bangya-slides). |
| [`mac-app-icon-recolor`](mac-app-icon-recolor/) | Recolors a macOS app icon (hue shift or palette swap) and attaches it as a Finder custom icon so the signed bundle stays intact. Renders a contact sheet to choose from, handles the App Management permission block, verifies the icon macOS actually resolves, and leaves `apply.sh`/`revert.sh` behind. Distilled from telling Feishu apart from Lark. |
